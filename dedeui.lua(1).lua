...
local library = {
    ...
    colors = {},
    folders = {
        main = "CHINESEFRAUDWARE.INTERNATIONAL",
        assets = "CHINESEFRAUDWARE.INTERNATIONAL/assets",
        configs = "CHINESEFRAUDWARE.INTERNATIONAL/configs"
    },
    shared = {
        initialized = false,
        fps = 0,
        ...
        function window:Watermark(info)
            ...
            utility:Connection(rs.RenderStepped, function(FPS)
                ...
                if (tick() - Tick) > 0.15 then
                    watermark_title.Text = string.format("CHINESEFRAUDWARE.INTERNATIONAL | Ping : %s | Fps : %u", library.shared.ping, library.shared.fps)
                    window.watermark:UpdateSize()
                    --
                    Tick = tick()
                end
